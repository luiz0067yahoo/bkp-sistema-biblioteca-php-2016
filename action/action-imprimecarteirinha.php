...
<body>
	...
	<?php
		...
		function verifica_ultimo ( $id_ultimo , $id_escola ){
			...
			$queryf = "SELECT ativo FROM Aluno WHERE id_aluno = '".$id_ultimo."';";
		...
		{
			$query_quantidade = "SELECT MAX(id_aluno) FROM Aluno where id_escola = '".$id_escola_atual."'";
			$result_quantidade = $conn->query($query_quantidade);
			$row_quantidade = $result_quantidade->fetch_assoc();
			$ultimo_registro = $row_quantidade["MAX(id_aluno)"];
    ...
    <div class="one">
			...
			{
				$query = "SELECT  Aluno.nome as nomealuno, Escola.telefone as telefone, Escola.nome as escola FROM Aluno JOIN Escola ON Aluno.id_escola = Escola.id_escola where id_aluno = '$codigos[$i]';";
				
				$result = $conn->query($query);
				if ($result->num_rows > 0) {
   				 // output data of each row
	    			while($row = $result->fetch_assoc()) {
	       				 $nome_aluno = $row["nomealuno"];
	       				 $escola = $row["escola"];
			...
			echo '<center><p id="p03"> CARTEIRA DE IDENTIFICAÇÃO - BIBLIOTECA </p></center>';
			echo '<p id="p01">&nbsp&nbsp Nome: ' . $nome_aluno . '</p>';
			echo '<center><img src="barcode.php?f=png&s=code-128&w=300&ts=5&h=85&th=12&d='.$codigos[$i].'"> </center> </div> ';
			}	
